...
for station_id in observed_df.index:
    ...
    predicted_data = predicted_df.loc[station_id, :].values
    ...
    plt.plot(probabilities, predicted_data[1:])
    plt.legend(["Observed data", "Fitted data", "Predicted data (linear reg.)"])
    plt.show()
